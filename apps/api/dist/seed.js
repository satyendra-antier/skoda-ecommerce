...
function imageUrls(sku, count = 3) {
    return Array.from({ length: count }, (_, i) => `https://picsum.photos/seed/${encodeURIComponent(sku)}-${i}/600/450`);
}
function categoryFromSku(sku) {
    const u = sku.toUpperCase();
    if (u.includes('MUG') || u.includes('BAG') || u.includes('BOTTLE') || u.includes('BLANKET') || u.includes('COOLER') || u.includes('SCARF') || u.includes('TSHIRT') || u.includes('JACKET') || u.includes('CAP') || u.includes('HAT') || u.includes('BEANIE') || u.includes('SOCKS') || u.includes('GLOVES'))
        return 'Lifestyle';
    if (u.includes('MAT') || u.includes('COVER') || u.includes('AIRFRESH') || u.includes('STEERING'))
        return 'Interior';
    if (u.includes('SUNSHADE') || u.includes('UMBRELLA'))
        return 'Exterior';
    if (u.includes('PHONE') || u.includes('LAPTOP') || u.includes('CABLE') || u.includes('POWERBANK') || u.includes('SPEAKER') || u.includes('TORCH') || u.includes('WATCH'))
        return 'Technology';
    return 'Accessories';
}
const SEED_PRODUCTS = [
...
async function seed() {
    ...
    const settingRepo = ds.getRepository(site_setting_entity_1.SiteSetting);
    await settingRepo.upsert({ key: 'categories', value: JSON.stringify(DEFAULT_CATEGORIES) }, ['key']);
    console.log('Seed: Categories ensured.');
    const repo = ds.getRepository(product_entity_1.Product);
